subroutine scheme_weno7th_1d_character_l(l,r,fluxp,fluxl,nl,il,bfsize)
use flow_1d,only:ss
	implicit none
	double precision,intent(in),dimension(nl,nl,il+2*bfsize) :: l,r
	double precision,intent(in),dimension(il+2*bfsize,nl) :: fluxp

	double precision,intent(out),dimension(nl,il+2*bfsize) :: fluxl
	integer :: i,j,k,n,m,nl,il,bfsize
	...
	do i=bfsize,il+bfsize
		do j=1,nl
			...
			temp1(j)=w0*q40+w1*q41+w2*q42+w3*q43
		enddo
		do j=1,nl
...
subroutine scheme_weno7th_1d_character_r(l,r,fluxn,fluxr,nl,il,bfsize)
use flow_1d,only:ss
implicit none
	double precision,intent(in),dimension(nl,nl,il+2*bfsize) :: l,r
	double precision,intent(in),dimension(il+2*bfsize,nl) :: fluxn

	double precision,intent(out),dimension(nl,il+2*bfsize) :: fluxr
